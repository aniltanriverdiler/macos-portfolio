...
const WEB_PROJECTS: LocationChild = {
  ...
  children: [
    ...
    {
      id: 9,
      name: "Pigment Studio AI",
      name_tr: "Pigment Studio AI Projesi",
      icon: "/images/folder.png",
      kind: "folder",
      ...
      children: [
        {
          id: 1,
          name: "Pigment Studio AI Project.txt",
          name_tr: "Pigment Studio AI Projesi.txt",
          icon: "/images/txt.png",
          kind: "file",
          fileType: "txt",
          position: "top-40 left-20",
          description: [
            "Pigment Studio is a Next.js + TypeScript AI image restyling app — you upload a photo, pick a style preset, and get a reimagined version back without losing the subject, pose, or framing of the original.",
            "The studio is a single workbench: drop in a JPG/PNG/WEBP, pick from curated presets like Storybook 3D, Anime Cel, Clay Render, or Voxel Block, choose a model, and watch the result come back. Aspect ratios are inferred from the source so the output stays model-friendly, and every run is saved to your history with the source URL, result URL, style, and prompt metadata so you can reopen anything later.",
            "Auth is handled by Clerk, which gates the studio routes and the generation/upload API endpoints. Uploads go through scoped tokens to ImageKit so users only touch their own assets. Plans come with monthly generation limits — quota is checked before the OpenAI call fires, and prompts use baked-in fidelity constraints to keep identity and composition from drifting across styles.",
            "Stack is Next.js 16 App Router, Tailwind 4, Shadcn/UI, and Motion on the front; AI SDK + OpenAI for generation, Drizzle ORM on Neon Postgres for history and quota tracking, and Sentry for spans and error logs around the generation lifecycle. Deployed on Vercel.",
          ],
          description_tr: [
            "Pigment Studio, Next.js + TypeScript ile yazılmış bir AI görsel yeniden stilleme uygulaması — bir fotoğraf yüklüyorsun, stil preseti seçiyorsun ve orijinalin konusunu, pozunu ve kadrajını koruyarak yeniden yorumlanmış halini geri alıyorsun.",
            "Stüdyo tek bir tezgâh gibi çalışıyor: JPG/PNG/WEBP yüklüyor, Storybook 3D, Anime Cel, Clay Render veya Voxel Block gibi hazır presetlerden birini seçiyor, modeli belirliyor ve sonucu bekliyorsun. Aspect ratio kaynağa göre otomatik çıkarılıyor; her üretim de geçmişe kaynak/sonuç URL'i, stil ve prompt metadata'sıyla birlikte düşüyor, istediğinde geri açıp bakabiliyorsun.",
            "Auth tarafı Clerk ile çözülüyor; stüdyo sayfaları ve üretim/upload API uçları arkasına saklı. Yüklemeler ImageKit'e scope'lu token'larla gidiyor, yani kullanıcı sadece kendi varlıklarına dokunabiliyor. Planlara aylık üretim limiti bağlı — kota OpenAI çağrısı atılmadan kontrol ediliyor, prompt'larda da kimlik ve kompozisyonun stilden stile kaymaması için sabit kısıtlar gömülü.",
            "Stack tarafında önde Next.js 16 App Router, Tailwind 4, Shadcn/UI ve Motion var; üretim için AI SDK + OpenAI, geçmiş ve kota takibi için Neon Postgres üzerinde Drizzle ORM, üretim akışının log ve span'leri için de Sentry. Yayın Vercel'de.",
          ],
        },
        {
          id: 2,
          name: "pigmentstudio.ai",
          icon: "/images/safari.png",
          kind: "file",
          fileType: "url",
          href: "https://pigment-studio.vercel.app/",
          position: "top-35 right-15",
        },
        {
          id: 4,
          name: "pigment-studio.png",
          icon: "/images/image.png",
          kind: "file",
          fileType: "img",
          position: "top-90 right-90",
          imageUrl: "/images/pigment-studio.png",
        },
        {
          id: 5,
          name: "Design.fig",
        ...
        },
        {
          ...
          kind: "file",
          fileType: "url",
          href: "https://github.com/aniltanriverdiler/pigment-studio.git",
          position: "top-5 right-50",
        },
      ],
    },
    ...
    {
      id: 9,
      name: "Margin Notes App",
      name_tr: "Margin Notes App Projesi",
      icon: "/images/folder.png",
      kind: "folder",
      ...
      children: [
        {
          id: 1,
          name: "Margin Notes App Project.txt",
          name_tr: "Margin Notes App Projesi.txt",
          icon: "/images/txt.png",
          kind: "file",
          fileType: "txt",
          position: "top-26 left-10",
          description: [
            "MarginNotes is a Next.js + TypeScript app where you upload a PDF book and basically talk to it — the assistant answers in voice using what’s actually in your text, not random internet fluff.",
            "You add a book with a cover (or it grabs the first page if you skip the image), the PDF gets parsed into searchable chunks in MongoDB, then Vapi runs a real-time voice session with a live transcript. There’s a notes panel per book plus a global notes page so you can jot things down and find them later.",
            "Clerk handles sign-in and the pricing table for plans that cap how many books you can keep and how long voice sessions can run. Uploads go through Vercel Blob with a gated token flow, and when the assistant needs a quote from the book it hits a search endpoint wired as a Vapi tool — so “what did the author say about X?” actually pulls from your segments.",
            "The UI is that dark “Violet Bloom” SaaS vibe — Tailwind 4, shadcn-style components, Motion on the hero and a few flourishes, Sonner toasts when things save or fail. Server actions do the heavy lifting for books, segments, sessions, and notes; forms lean on Zod + react-hook-form.",
          ],
          description_tr: [
            "MarginNotes, Next.js + TypeScript ile yazılmış bir uygulama — PDF kitabını yüklüyorsun ve aslında kitapla konuşuyormuş gibi sesli sohbet ediyorsun; asistan cevapları kitaptaki metne dayanıyor, rastgele internet cevabı değil.",
            "Kitabı kapakla birlikte ekliyorsun (kapak yoksa ilk sayfa kapak oluyor), PDF taranıp MongoDB’de aranabilir parçalara bölünüyor, sonra Vapi ile gerçek zamanlı ses oturumu ve canlı transcript geliyor. Her kitap için not alanı var, ayrıca tüm notları tek yerden gördüğün bir sayfa da var.",
            "Giriş ve fiyatlandırma Clerk’te; planlar kaç kitap tutabileceğini ve ses oturumu sürelerini sınırlıyor. Dosyalar Vercel Blob’a, token’lı güvenli upload akışıyla gidiyor. Asistan kitaptan alıntı isteyince Vapi aracı `/api/vapi/search-book` üzerinden segment araması yapıyor — yani “yazar X’i nerede anlatıyor?” gerçekten senin metninden geliyor.",
            "Arayüz koyu “Violet Bloom” SaaS havasında — Tailwind 4, shadcn tarzı bileşenler, hero ve birkaç yerde Motion, kayıt/hata için Sonner toast’ları. Kitap, segment, oturum ve not işleri server action’larla; formlarda Zod + react-hook-form var.",
          ],
        },
        {
          id: 2,
          name: "marginnotes.app",
          icon: "/images/safari.png",
          kind: "file",
          fileType: "url",
          href: "https://margin-notes.vercel.app/",
          position: "top-35 right-15",
        },
        {
          id: 4,
          name: "margin-notes.png",
          icon: "/images/image.png",
          kind: "file",
          fileType: "img",
          position: "top-90 right-90",
          imageUrl: "/images/margin-notes.png",
        },
        {
          id: 5,
          name: "Design.fig",
  ...
  children: [
    ...
    // ▶ Mobile Project 3
    {
      ...
      children: [
        {
          id: 1,
          name: "Garnish App Project.txt",
          name_tr: "Garnish App Projesi.txt",
          icon: "/images/txt.png",
          kind: "file",
          fileType: "txt",
          position: "top-32 right-40",
          description: [
            "Garnish is a full-stack food ordering app — Expo + React Native on the phone, an Express + Prisma API on the backend, and a shared package in between so types and validation don’t drift between the two sides.",
            "On mobile you browse restaurants and categories, search products with a debounced query, and open a detail screen to pick toppings and sides before dropping things into the cart. The cart lives on the server (Prisma + PostgreSQL), so it survives logout/login; optimistic updates keep the tab bar badge in sync the moment you tap.",
            "Auth is JWT with an access + refresh pair. The Axios client catches 401s, refreshes the token quietly, and replays the original request — no “please sign in again” popup. Orders go through server-side price validation with nested Prisma writes, so the client can’t quietly forge totals.",
            "The repo is a small monorepo: apps/api, apps/mobile, and packages/shared for the Zod schemas, types, and enums both sides import. State is split — Zustand for auth and cart on the client, React Query for server data. NativeWind v4 + Expo Router on the surface, Zod-validated env on the API, errors piped through Sentry.",
          ],
          description_tr: [
            "Garnish, telefonda Expo + React Native, arkada Express + Prisma API ve iki tarafın paylaştığı bir shared paketten oluşan tam yığın bir yemek sipariş uygulaması; tipler ve doğrulama iki tarafta da aynı kaynaktan geliyor, kayma olmuyor.",
            "Mobilde restoranlar ve kategoriler arasında geziniyorsun; arama debounce’lu, ürün detayında topping ve side seçip sepete atıyorsun. Sepet sunucuda (Prisma + PostgreSQL) duruyor, çıkış/giriş yapsan da silinmiyor; optimistic update’ler tab bar üstündeki sayıyı sen dokunur dokunmaz güncel tutuyor.",
            "Auth tarafında JWT access + refresh ikilisi var. Axios interceptor 401 yakalayınca sessizce token tazeliyor ve isteği yeniden atıyor — “tekrar giriş yap” popup’ı yok. Siparişler sunucuda fiyat doğrulamasından geçip iç içe Prisma yazımıyla kaydediliyor, yani client toplamları sessizce şişiremiyor.",
            "Repo küçük bir monorepo: apps/api, apps/mobile ve iki tarafın da kullandığı Zod şemaları, tipler ve enum’lar için packages/shared. State ikiye ayrılmış — auth ve sepet için Zustand, sunucu verisi için React Query. Önde NativeWind v4 ve Expo Router; API’da env Zod ile doğrulanıyor, hatalar Sentry’e düşüyor.",
          ],
        },
        {
          id: 2,
          name: "garnish-app.com",
          icon: "/images/safari.png",
          kind: "file",
          fileType: "url",
          href: "https://github.com/aniltanriverdiler/garnish-app.git",
          position: "top-20 right-95",
        },
        {
          id: 4,
          name: "garnish-app.png",
          icon: "/images/image.png",
          kind: "file",
          fileType: "img",
          position: "top-80 right-35",
          imageUrl: "/images/garnish-app.png",
        },
        {
          id: 5,
          name: "Design.fig",
